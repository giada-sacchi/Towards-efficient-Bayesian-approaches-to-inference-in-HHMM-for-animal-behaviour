...
pn <- rapply(pn, function(x) round(x, digits=1), how="replace") # approximate

n.iter <- 16000 # number of iterations
n.tune <- 6000 # number of iterations for tuning
...
output <- updatepar.MH1(par.list=pn, npar=npar, data=data, likhood=likelihood, 
                      alpha=alpha, beta=beta, delta=delta, mu=mu, sigma=sd,
                      shape=shape, rate=rate, n.iter, tuning=T, thr=thr, n.tune=n.tune)
  
# Save output
itns.MH1 <- output$itns[31:51,]
...
# Save estimates
estimates <- rowMeans(itns.MH1[,(n.iter-n.tune+1):n.iter])
